...
\begin{enumerate}
\item \textit{Definition:} 
\item \textit{The Concept:} Borkin et al\cite{borkin2013makes} built an online memorability study over 2000 static visualization that covers the large variety of visualizations, and determine which visualization types and attributes are more memorable. It  opens a new domain of investigation at the interface between human cognition and visualization design.
\item \textit{Case Study:} 
\begin{enumerate}
\item Visualization taxonomy: The taxonomy classifies static visualizations according to the underlying data structures, the visual encoding of the data, and the perceptual tasks enabled by these encodings. It contains twelve main visualization categories and several popular sub-types for each category.
\item Memorability experiment: Borkin et al run memorability test via Amazon’s Mechanical Turk with 261 participants and gather memorability score.
...
\textbf{Title:} \textit{Optimized Comics-Based Storytelling for Temporal Image Sequences}
\begin{enumerate}
\item \textit{Definition:} Stories are sequences of causally related events presented in text, images, graphics, or videos, and storytelling can be manifested in various forms \cite{chu2015}.
\item \textit{The Concept:} This paper describe a system to transform any temporal image sequence to a comics-based storytelling visualization \cite{chu2015}. 
\item \textit{System processing:}
\begin{enumerate}
\item Optimized Page Allocation: this is used to segment sequence into subsequence automatically, and same subsequence is allocated into the same comics page \cite{chu2015}.
\item Optimized Layout Selection: This step selects the best layout based on the number of images to be displayed \cite{chu2015}.
\item Optimized Speech Balloon placement: This process selects speech balloon sizes and place them into an optimized place to combine language ingredient of storytelling with comics-based visualization. See Figure \ref{chu2015} \cite{chu2015}.
\end{enumerate}
\item \textit{Related Work:} This paper is based on previous work of Chu et al \cite{chu2012,chu2013optimized},develops a system that can automatically generate comics-based storytelling visualization from temporal image.
\item \textit{In this paper, the following visualization techniques are illustrated :} 
\begin{enumerate}
\item Sphere placement, Label placement, page placement
...
\begingroup
\centering
\includegraphics[width=8cm]{./images/chu2015}
\captionof{figure}{\textit{This figure shows the output of comics-based storytelling visualization \cite{chu2015}.}}\label{chu2015}
\endgroup
